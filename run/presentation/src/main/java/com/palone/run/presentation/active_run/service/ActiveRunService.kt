package com.palone.run.presentation.active_run.service
...
class ActiveRunService : Service() {
    ...
    private val baseNotification by lazy {
        NotificationCompat.Builder(applicationContext, CHANNEL_ID).setSmallIcon(
            com.palone.core.presentation.designsystem.R.drawable.logo
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                val activityClass = intent.getStringExtra(EXTRA_ACTIVITY_CLASS)
                    ?: throw IllegalArgumentException("Activity class is missing")
        ...
        return START_STICKY//important to keep the service running
    }
    ...
    private fun start(activityClass: Class<*>) {
        if (!isServiceActive) {
            ...
            val notification =
                baseNotification.setContentInfo("00:00:00").setContentIntent(pendingIntent).build()
    ...
    fun stop() {
        ...
        serviceScope =
            CoroutineScope(Dispatchers.Main + SupervisorJob())//to prevent relaunch of the service, so it wont use the old scope with old data
    }
